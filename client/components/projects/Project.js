...
class Project extends Component {
  ...
  onCreateTask = (e, card) => {
    ...
    this.props.createTask(this.props.match.params.id, {
      ...
      responsibility: this.state.responsibility, // new attribute ... 
      priority: this.state.priority,        // new attribute ... 
      //progress:'',
      completed: false,
      card
    });
      ...
      <div className="container">
        ...
        <div className="row" style={{ margin: '16px 0' }}>
          <div className="col" style={{ padding: '0' }}>
            <a href="#" className="btn btn-sm btn-primary" onClick={this.onCreateCard}>Add new Category</a>
            <input
              style={{ width: '200px' }}
              type="text"
            ...
            <ReactCSSTransitionGroup {...cardsTransitionOptions}>
              {
                uicards
                  ...
                  .filter(card => {
                    ...
                    return (regex.exec(card.name) || card.tasks.find(task => regex.exec(task.responsibility, task.description, task.priority)));
                  })
                  .map(card => (
                    <Card
                      key={card._id}
                          ...
                          card.tasks
                            ...
                            .map(task => (
                              <TasksListItem
                                ...
                                onDescriptionChanged={(description, responsibility, priority) => { this.props.updateTask(id, task._id, { description, responsibility }) }}
                                onResponsibilityChanged={(responsibility) => { this.props.updateTask(id, task._id, { responsibility }) }}
